 ...
 * (Protégé par try/catch : si la colonne "certifie" n'existe pas encore
 * — migration non exécutée —, on ignore simplement, sans casser la
 * délivrance du certificat elle-même.)
 */
function verifierCertificationAutoPromoteur(\PDO $db, int $promoteurId): void {
    try {
        ...
        // Silencieux : ne doit jamais faire échouer la délivrance du certificat.
    }
}
...
function etudiantsTermineModule(): void {
    ...
    try {
        ...
        repondreJSON(['succes' => false, 'message' => 'Erreur lors du chargement des étudiants.'], 500);
    }
}
...
function verifierCertificationAutoEnseignant(\PDO $db, int $coursId): void {
    try {
        ...
        // Silencieux : ne doit jamais faire échouer la validation de la leçon.
    }
}
